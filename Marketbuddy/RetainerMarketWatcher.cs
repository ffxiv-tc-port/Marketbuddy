...
namespace Marketbuddy
{
    ...
    /// <para>🔑 同一個形狀順帶解決了重掛：改價只是把某個價位的件數搬到另一個價位，
    /// <b>(道具, 品質) 的總件數不變</b>，所以批次重掛不會生出任何假的「賣出」事件。</para>
    /// </summary>
    ...
    /// <para>存活時間刻意短（<see cref="TtlMinutes"/> 分鐘）：這本帳是用來解釋<b>剛剛</b>發生的事，
    /// 留太久只會讓一次很久以前的下架去認領一筆真正的賣出。過期沒被認領的條目自然消失，
    /// 失敗形式是「那一列變成低信心」——不會產生錯誤的高信心結論。</para>
    /// <para>執行緒：所有呼叫點都在 framework 執行緒上（兩個下架引擎與觀察者都掛在
    /// <c>Framework.Update</c> 上），但仍然上鎖——這種清單一旦哪天被別的路徑碰到，
    /// 裸 <c>List</c> 的失敗形式是清單本身壞掉，不是拿到舊值。</para>
    /// </summary>
    ...
    /// <para>⚠️ 這個檔是<b>整份重寫</b>（僱員會被解僱、清單會縮短，追加寫不出「某一列不見了」），
    /// 走「暫存檔 → <see cref="File.Move(string,string,bool)"/>」，所以寫到一半崩潰不會留下
    /// 半份基準線。形狀與存檔紀律逐字比照 <see cref="PendingActions"/>。</para>
    /// </summary>
    ...
    /// <para>🔑 <b>為什麼只在「出售品」視窗開著時取樣。</b>市場容器是全域的一份，而
    /// <c>GetActiveRetainer()</c> 換人的時機與容器被伺服器填好的時機不保證同步；
    /// 兩者錯開的那幾幀會讓僱員 B 的容器內容被拿去跟僱員 A 的基準線比對，
    /// 結果是<b>一整批假的「賣出」</b>。出售品視窗開著的期間，容器屬於誰是確定的。</para>
    /// <para>🔑 <b>兩次相同的讀取才算數。</b>視窗剛開的那幾幀容器可能還沒填好，
    /// 「還沒填好」與「東西全賣光了」長得一模一樣。所以先拍成候選，隔一段時間再讀一次，
    /// 內容（含錢包）完全相同才承認它是一份快照。代價只是慢半秒。</para>
    /// <para>執行緒：全部在 framework 執行緒上（<c>Framework.Update</c>）。落地一律交給
    /// <see cref="RetainerSalesLog"/> 與 <see cref="RetainerMarketState"/>，那兩支自己
    /// 把 I/O 丟到執行緒池，鎖內不做 I/O。</para>
    /// </summary>
