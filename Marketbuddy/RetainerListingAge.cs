...
namespace Marketbuddy
{
    ...
    /// </list>
    /// <para>🔴🔴 <b>暫定值只在「完全沒有任何紀錄」時寫一次。</b>寫成「每次載入都蓋成載入時間」的話，
    /// 每次重開遊戲年齡就歸零 ⇒ 永遠顯示「剛掛上去」，而且看起來完全正常（有數字、會變、不報錯）。
    /// 判準只有一條：<b>有紀錄就不要動它</b>。</para>
    /// <para>🔑 <b>比對鍵是 <c>(僱員, 道具, 品質)</c>，刻意不含價格也不含件數</b>：
    /// 重掛只是換價格、賣掉一部分只是件數變少，兩者都是「同一批東西還在架上」，
    /// 把它們算成重新開始會讓這個數字失去意義。所以
    /// <b>「掛 3 件賣掉 1 件」之後剩下的 2 件沿用原本的起點</b>；
    /// 只有整個 <c>(僱員, 道具, 品質)</c> 從架上消失、之後又重新出現時才會重新計時。</para>
    /// <para>⚠️ 代價是：同一件道具分兩次掛上去（第一批還沒賣完就又補掛），第二批會沿用第一批的起點。
    ...
    /// 真正的 I/O 在鎖外、在執行緒池上。<see cref="Observe"/> 只從 framework 執行緒呼叫。</para>
    /// </summary>
